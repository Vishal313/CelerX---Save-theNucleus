...
function binary_to_base64(e) {
  for (var t = new Uint8Array(e), r = new Array(), n = 0, i = 0, a = new Array(3), o = new Array(4), s = t.length, d = 0; s--;) if (a[n++] = t[d++], 3 == n) {
  ...
  if (!window._dsf) {
    var e,
      ...
      _handleMessageFromNative: function (e) {
        ...
        if (n) a(n, this._dsf);else if (i) o(i, this._dsaf);else if (n = e.method.split("."), !(2 > n.length)) {
          e = n.pop();
          var n = n.join("."),
              i = this._dsf._obs,
              i = i[n] || {},
...
/* harmony import */ var _css_main_css__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(313);
/* harmony import */ var _css_main_css__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_css_main_css__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _css_reset_css__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(316);
...
/* harmony import */ var _celerx__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(60);
/* harmony import */ var _celerx__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_celerx__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var seed_random__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(124);
/* harmony import */ var seed_random__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(seed_random__WEBPACK_IMPORTED_MODULE_3__);
...
var Core = new function () {
  ...
  var DEFAULT_WIDTH = 1000,
      DEFAULT_HEIGHT = 650,
      BORDER_WIDTH = 6,
      FRAMERATE = 60; // Types of organisms

  var ORGANISM_ENEMY = 'enemy',
      ORGANISM_ENERGY = 'energy'; // The world dimensions
  ...
  var canvas, context;
  var canvasBackground, contextBackground; // UI DOM elements

  var status;
  var panels;
  var message;
  var title;
  var startButton; // Game elements

  var organisms = [];
  var particles = [];
  var player; // Mouse properties

  var mouseX = (window.innerWidth + world.width) * 0.5;
  var mouseY = (window.innerHeight + world.height) * 0.5;
  var mouseIsDown = false;
  var spaceIsDown = false; // Game properties and scoring
  ...
  var duration = 0;
  var difficulty = 1;
  var lastspawn = 0; // Game statistics
  ...
  this.init = function () {
    ...
    if (canvas && canvas.getContext) {
      ...
      contextBackground = canvasBackground.getContext('2d'); // Register event listeners
      ...
      startButton.addEventListener('click', startButtonClickHandler, true);
      document.addEventListener('keydown', documentKeyDownHandler, false);
      document.addEventListener('keyup', documentKeyUpHandler, false); // Define our player

      player = new Player(); // Force an initial resize to make sure the UI is sized correctly

      windowResizeHandler(); // If we are running on mobile, certain elements need to be configured differently
  ...
  function renderBackground() {
    var match = _celerx__WEBPACK_IMPORTED_MODULE_2___default.a.getMatch();
    seed_random__WEBPACK_IMPORTED_MODULE_3___default()(match && match.sharedRandomSeed, {
      global: true
    });
    _celerx__WEBPACK_IMPORTED_MODULE_2___default.a.start();
  ...
  function startButtonClickHandler(event) {
    //celerx.start();
    if (playing == false) {
      playing = true; // Reset game properties

      organisms = [];
      score = 0;
      difficulty = 1; // Reset game tracking properties

      fc = 0;
      fs = 0; //ms = 0;

      cs = 0; // Reset the player data

      player.energy = 30; // Hide the game UI
  ...
  function gameOver() {
    playing = false; // Determine the duration of the game
    //duration = new Date().getTime() - time;
    //	panels.style.display = 'block';

    score = Math.round(score);
    _celerx__WEBPACK_IMPORTED_MODULE_2___default.a.submitScore(score);
    title.innerHTML = 'Game Over! (' + score + ' points)'; //scoreText = 'Score: <span>' + Math.round( score ) + '</span>';
    //scoreText += ' Time: <span>' + Math.round( ( ( new Date().getTime() - time ) / 1000 ) * 100 ) / 100 + 's</span>';
    //tatus.innerHTML = scoreText;
  }
  ...
  function windowResizeHandler() {
    ...
    world.height = isMobile ? window.innerHeight : DEFAULT_HEIGHT; // Center the player

    player.position.x = world.width * 0.5;
    player.position.y = world.height * 0.5; // Resize the canvas

    canvas.width = world.width;
    canvas.height = world.height;
    canvasBackground.width = world.width;
    canvasBackground.height = world.height; // Determine the x/y position of the canvas

    var cvx = (window.innerWidth - world.width) * 0.5;
    var cvy = (window.innerHeight - world.height) * 0.5; // Position the canvas
  ...
  function animate() {
    ...
    var frameTime = new Date().getTime(); // Increase the frame count

    frames++; // Check if a second has passed since the last time we updated the FPS
    ...
    var scoreFactor = 0.01 + Math.max(Math.min(fps, FRAMERATE), 0) / FRAMERATE * 0.99; // Scales down the factor by itself

    scoreFactor = scoreFactor * scoreFactor; // Clear the canvas of all old pixel data

    context.clearRect(0, 0, canvas.width, canvas.height);
    var i, ilen, j, jlen; // Only update game properties and draw the player if a game is active

    if (playing) {
      // Increment the difficulty slightly
      difficulty += 0.0015; // Increment the score depending on difficulty

      score += 0.4 * difficulty * scoreFactor; // Increase the game frame count stat

      fc++; // Increase the score count stats

      fs += 0.4 * difficulty * scoreFactor; //player.angle = Math.atan2( mouseY - player.position.y, mouseX - player.position.x );
    ...
    if (spaceIsDown && player.energy > 10) {
      ...
      context.fill();
      context.stroke(); // play sound for the shield
      //CoreAudio.playShield();
    }

    var enemyCount = 0;
    var energyCount = 0; // Go through each enemy and draw it + update its properties
    ...
    for (i = 0; i < particles.length; i++) {
      p = particles[i]; // Apply velocity to the particle

      p.position.x += p.velocity.x;
      p.position.y += p.velocity.y; // Fade out

      p.alpha -= 0.02; // Draw the particle

      context.fillStyle = 'rgba(255,255,255,' + Math.max(p.alpha, 0) + ')';
      context.fillRect(p.position.x, p.position.y, 1, 1); // If the particle is faded out to less than zero, remove it
...
Energy.prototype = new Point(); // shim with setTimeout fallback from http://paulirish.com/2011/requestanimationframe-for-smart-animating/
